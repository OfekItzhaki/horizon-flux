...
async function verifyRegistration() {
    ...
    try {
        // 1. Start Registration
        console.log('1. Calling /auth/register/start...');
        const startRes = await axios.post(`${API_URL}/auth/register/start`, { email: TEST_EMAIL });
        console.log('   Response:', startRes.data);
        ...
        const regToken = verifyRes.data.registrationToken;
        console.log('   Registration Token received:', regToken.substring(0, 20) + '...');
        ...
        console.log('   Final Response (should include accessToken):', finishRes.data.accessToken ? 'SUCCESS (Token present)' : 'FAILED');

        // Cleanup lists first then user
        const createdUser = await prisma.user.findUnique({ where: { email: TEST_EMAIL } });
        if (createdUser) {
            await prisma.toDoList.deleteMany({ where: { ownerId: createdUser.id } });
            await prisma.user.delete({ where: { id: createdUser.id } });
        }
